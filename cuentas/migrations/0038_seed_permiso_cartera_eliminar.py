"""Permiso (cartera, eliminar) — borrado PERMANENTE de un cliente archivado,
distinto de archivar (S-Ajustes-Jul23 R2, Oscar).
...
nuevos según DEFAULTS_POR_ROL (donde solo super_admin lo trae vía failsafe).
...
def seed(apps, schema_editor):
    ...
    filas = [
        PermisoUsuario(usuario=u, modulo="cartera", permiso="eliminar", activo=True)
        for u in supers
    ]
    if filas:
...
def reverse(apps, schema_editor):
    PermisoUsuario = apps.get_model("cuentas", "PermisoUsuario")
    PermisoUsuario.objects.filter(modulo="cartera", permiso="eliminar").delete()


class Migration(migrations.Migration):
    dependencies = [("cuentas", "0037_seed_permiso_mcp")]
