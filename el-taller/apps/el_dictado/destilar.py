"""El Chalán que aprende de lo que ve (S-Chalan-Aprende-V1).
...
Trigger: `python manage.py chalan_destilar_aprendizajes [--dias N] [--dry-run]`
(cron semanal, ver crontab §10). NO es invocable por el usuario vía El Chalán
(es un proceso de back-office); el super_admin lo dispara/revisa en Gerencia.
"""
...
def _frases_existentes() -> set[str]:
    ...
    return {
        _norm(f)
        for f in DictadoAprendizaje.objects.values_list("frase_o_patron", flat=True)
        if f
    }
