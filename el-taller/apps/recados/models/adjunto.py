...
from .conversacion import Mensaje
...
class MensajeAdjunto(models.Model):
    """Archivo adjunto a un mensaje del chat de Recados, en Google Drive.

    Mismo patrón que RecadoAdjunto: guardamos la referencia y servimos el
    contenido por proxy autenticado, no con liga pública.
    """

    mensaje = models.ForeignKey(
        Mensaje, on_delete=models.CASCADE, related_name="adjuntos"
    )
    drive_file_id = models.CharField(max_length=255)
    nombre = models.CharField(max_length=255)
    mime_type = models.CharField(max_length=120, blank=True, default="")
    ...
    def __str__(self) -> str:
        return f"mensaje_adjunto#{self.pk} {self.nombre}"
